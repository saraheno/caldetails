...
void res() {
  ...
  filenames[0]="test_hist.root"; 


  double aatruemean[npoints];
  aatruemean[0]=10;  // generator level momentum for file.  assumes signle eneryg
  ...
  double abc,dej;
  for(int k=0;k<nhst;k++) { // different energy estimates
    for(int j=0;j<npoints;j++){  // total number of data files
      std::cout<<"fitting "<<hnam[k]<<" at energy "<<aatruemean[j]<<std::endl;
      resolution(filenames[j],hnam[k].c_str(),&abc,&dej);
      aaamean[j][k]=abc;
      aarms[j][k]=dej;
      rrres[j][k]=0;
      if(abc!=0) rrres[j][k]=aarms[j][k]/abc;  //fractional resolution
    }
  }
  ...
  TF1 *calice = new TF1("calice","sqrt(0.57*0.57/x+0.016*0.016)",5,100);  // arXiv:1507.05892 taken from pg 21 but needs to be checked.  resolutio of calice calorimeter


  // plot for first energy estimate
  double arrres[npoints];
  for (int k=0;k<npoints;k++) {arrres[k]=rrres[k][0];}
  auto g1 = new TGraph(npoints,aatruemean,arrres);
  g1->Fit("f2");

  // plot for second energy estimate
  ...
  g2->Draw("CP");
  lgd->AddEntry(g2, "total energyy in calorimeter and escaping ke", "l");
